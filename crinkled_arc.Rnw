...
non-overlapping.  \cite{halmos1982} interprets such a curve as making
a sudden right angle turn at each point, justifying his statements
that (infinite-dimensional) Hilbert space is ``even roomier than it
looks''.  \cite{halmos1982} goes on to make the observation that every
...
scaling, and inclusion.  We now verify numerically that the Halmos arc
...
ylow <- -20
yhigh <- -10
points(cbind(0.1,seq(from=yhigh,to=ylow,len=100)),pch=15,col=rainbow(100))
thing <- function(y,a,b,c,d,xa=0.2,xb=0.7){
  segments(x0=xa+a*(xb-xa), x1=xa+b*(xb-xa),y0=y)
  segments(x0=xa+c*(xb-xa), x1=xa+d*(xb-xa),y0=y+0.01)
}

nn <- 10
for(i in 0:nn){
  thing(y=yhigh + i/nn*(ylow-yhigh), a=0.1, b=0.3, c=0.2 + (0.3)*i/nn, d=0.4 + (0.3)*i/nn)
}
@
